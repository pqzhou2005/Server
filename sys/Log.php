 ...
 * $Id: Log.php 13 2015-02-02 06:27:31Z zhoupengqian $ 
 */
namespace sys;
...
class Log
{
	...
	{
		$this->logPath =  \sys\Config::getInstance()->log_file;
	}
	...
	public function write()
	{
		...
		if ($filepath)
		{
			...
			{
				$create = true;
				$name = (!empty($_SERVER['argv']) && !empty( $_SERVER['argv'][0]))? $_SERVER['argv'][0] : $_SERVER['SCRIPT_FILENAME'];
			
				$data = "# LogFrom:" .  $name ." LogSetPath:". $this->path ." #\n\n" . $data;
			}
	...
	{
		array_unshift($this->data, date("Y-m-d H:i:s"));
	...
	private function pathValidate($path)
	{
		...
		for ($i = 0; $i < count($arraypath); $i++)
		{
			...
			if (!is_dir($tmppath)) 
			{
				$old = umask(0);// 设置权限，不然在cmd模式下面，创建的模式为755，www用户不能写了
